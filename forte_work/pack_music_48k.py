#!/usr/bin/env python3
"""Pack Separado_Music stems -> 9 tracks 48k looped to 60min -> DATA_Forte.js."""
import base64
import glob
import os
import re
...
MP3_DIR = os.path.join(OUT_DIR, "mp3_music_48k")
TARGET_SEC = 3610.0
BITRATE = "48k"
...
REPEATS = 13  # 5:07 * 13 ≈ 66min > 3610s
...
def write_concat_list(src_path, list_path):
    real = os.path.abspath(src_path).replace("\\", "/")
...
def encode_track(name, src_path):
    ...
    if os.path.isfile(dst) and os.path.getsize(dst) > 20_000_000:
        try:
            d = duration(dst)
            if abs(d - TARGET_SEC) <= 3.0:
                print("  skip %s (already %d bytes, %.2fs)" % (name, os.path.getsize(dst), d), flush=True)
                return dst
        except Exception:
            pass
    p = PROCESS[name]
    ...
    af = ",".join(filters)
    list_path = os.path.join(MP3_DIR, name + ".txt")
    write_concat_list(src_path, list_path)
    cmd = [
        "ffmpeg", "-y",
    ...
    print(" ", name, "<-", os.path.basename(src_path), "vol=%.2f" % p["vol"], BITRATE, "loop x%d" % REPEATS, flush=True)
